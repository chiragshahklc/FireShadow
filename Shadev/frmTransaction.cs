    ...
    public partial class frmTransaction : Form
    {
                    ...
                    case FrmCompany.TransAdd:
                        {
                            ...
                            else if (cmbTaxType.SelectedIndex < 0)
                            {
                                MessageBox.Show("Please Select Tax Type.", "ShadevInfotech", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                            else
                            {
        ...
        private void RefreshItems()
        {
            try
            {
                ...
                AIO.command = "select id,(select modName from Model where id=itgModID) as [Model],itgTranID,itgQTY as [QTY],Round((itgPrice*itgQTY),2) as [Price],Round((itgTotal*itgQTY),2) as [Total],itgDesc as [Desc] from TranItemsGrid " + query + " order by id ASC";
                var dt = a1.dataload();
